function menuAnimation() {
    ...
    // By default, the first one (index 0) is active for Works
    let active = 1;
    let currentRotation = 0;
    let previousActiveSecond = document.querySelector(".str1 .second"); // The first one (Works) by default

    // Set the first mncircle and the Works link to active by default
    let previousActiveCircle = document.querySelectorAll(".mncircle")[0]; 
    gsap.to(previousActiveCircle, {
        opacity: 1, // Make the first mncircle fully visible initially
        ease: Expo.easeInOut,
        duration: 1
    });

    // Make the "Works" link visible by default
